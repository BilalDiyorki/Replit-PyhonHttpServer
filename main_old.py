...
class MyRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == '/':
            ...
            user_table = '<tbody>'
            for user in users:
                user_table += f'<tr><td>{user["first_name"]}</td><td>{user["last_name"]}</td><td>{user["username"]}</td><td><button class="btn btn-primary copy-button">Copy</button></td></tr>'
            user_table += '</tbody>'
      
            context = {'user_table': user_table}
            content = render_template('home.html', context)
            self.wfile.write(content.encode())
